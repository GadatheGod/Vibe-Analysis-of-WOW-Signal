...
# --- Plot 1: Heatmap with LOG scale to show signals clearly ---
ax1 = axes[0, 0]
# Use log scale for color to make signals stand out from background
data_log = np.where(channel_data.values > 0, channel_data.values, 0.1)
cmap = LinearSegmentedColormap.from_list('wow', [(0, '#000033'), (0.2, '#003366'), (0.5, '#0099cc'), (0.7, '#ffcc00'), (1, '#ff0000')])
im1 = ax1.imshow(data_log, aspect='auto', cmap=cmap, interpolation='nearest', vmin=0.1, vmax=10)
ax1.axhline(y=wow_idx, color='white', linestyle='--', linewidth=2.5, label=f'WOW! Signal (22:09:25, Row 26)')
ax1.axhline(y=max_row, color='cyan', linestyle=':', linewidth=2.5, label=f'Peak Signal (22:16:10, Row 60)')
ax1.axhline(y=2, color='orange', linestyle='-.', linewidth=1, alpha=0.5, label='Background level (~2.0)')
...
ax1.set_title('Heatmap: All 50 Channels Over Time\n(Red = Signal > 2.0, White line = WOW! at Row 26)', fontsize=11)
ax1.legend(loc='upper right', fontsize=8)
plt.colorbar(im1, ax=ax1, label='Signal Strength (log scale)', shrink=0.8)
...
# Mark the hydrogen line channel
...
ax2.set_title(f'WOW! Signal at Peak (22:09:25)\nChannel 3 = {wow_row_data[2]:.1f} SNR', fontsize=11)
...
# Calculate approximate frequency for each channel
channel_freqs = []
for i in range(50):
    freq = 1420.4056 + (i - 25) * 0.01008
...
# Mark the hydrogen line channel (Channel 3)
...
plt.savefig('wow_signal_analysis_v3.png', dpi=200, bbox_inches='tight')
print("Saved: wow_signal_analysis_v3.png")
...
print(f"  Channel: {max_col + 1} (freq ~{channel_freqs[max_col]:.4f} MHz)")
...
print(f"\nHeatmap note: Red areas at 3-4 SNR are BACKGROUND NOISE, not signals.")
print(f"              Only the bright spike at Row 26 is the WOW! signal.")
